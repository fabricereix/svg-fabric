...
width v element@(Svg _ _ _ _) =
  case Parser.auto v of
      Right parsed -> Right $ Svg.width element $ OneOf3 parsed
      Left _ -> case Parser.length v of
          Right parsed -> Right $ Svg.width element $ TwoOf3 parsed
          Left _ -> case Parser.percentage v of
              Right parsed -> Right $ Svg.width element $ ThreeOf3 parsed
...
height v element@(Svg _ _ _ _) =
  case Parser.auto v of
      Right parsed -> Right $ Svg.height element $ OneOf3 parsed
      Left _ -> case Parser.length v of
          Right parsed -> Right $ Svg.height element $ TwoOf3 parsed
          Left _ -> case Parser.percentage v of
              Right parsed -> Right $ Svg.height element $ ThreeOf3 parsed
              Left _ -> Left $ "Can not parse value " ++ v
